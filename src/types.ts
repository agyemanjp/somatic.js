...
export type PropsExtended<Props, Msg extends Message = Message, State = {}> = Props & {
	...
	stateCache?: {
		getAsync: (key: string) => Promise<State>
		setAsync: (key: string, delta: Partial<State>) => Promise<any>
	}
...
export type Component<Props extends Obj = Obj, Msg extends Message = Message, State = {}> = (
	(props: PropsExtended<Props, Msg, State>) => JSX.Element
)
...
export type ViewProps<TData = Record<string, unknown>> = {
