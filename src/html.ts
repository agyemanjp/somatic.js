...
export function stringifyAttributes<E>(props: HTMLAttributes<any> & E): string {
    ...
    return Object.keys(props)
        .map((key) => {
            const effectiveKey = keys(attributeConversions).includes(
                key.toLowerCase()
            )
            ...
            // const htmlKey = encodeHTML(DashCase(key))
...
export function stringifyStyle(
    ...
    if (typeof style === "object") {
        return Object.keys(style)
            .map(
                (key) =>
                    `${dashCase(key)}: ${style[key as keyof typeof style]}${important === true ? " !important" : ""
                    }`
            )
            .join("; ")
        // .concat(";")
        ...
        console.warn(
            `Input "${JSON.stringify(
                style
            )}" to somatic.stringifyStyle() is of type ${typeof style}, returning empty string`
        )
        return ""
    }
}
