...
// 	// if (!type) console.warn(`Type argument mising in call to createElement, will treat as fragment`)
// 	return {
// 		type,
// 		props: props ?? {},
...
// 	if (isTextDOM(dom)) {
// 		return elt !== undefined // don't use hasValue() here since that will flag null, "", etc, which are valid value elements
...
// 				const x = isIntrinsicElt(trace.leafElement)
// 					? applyLeafElementAsync(dom, trace.leafElement).then(_ => Object.assign(_, { renderTrace: trace }))
// 					: (() => { updateDomShallow(dom, trace.leafElement); return Promise.resolve(dom) })()
...
// export async function mountElement(element: UIElement, container: Element, options?: MountOptions) {
...
// 	newChildren.flat().forEach(dom => fragment.appendChild(dom))
